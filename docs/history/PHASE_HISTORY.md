...
This file preserves the project’s development history and phase-by-phase design notes. For a first-time overview, start with the README.

## Phase 3: Ego Planner

The Ego stage introduced a deterministic reality-principle planner (`EgoRealityPlan`) that scores candidate strategies before Ego LLM realization. This plan is internal-only and must not include `U*`. Ego prioritizes user benefit, truthfulness, autonomy, and safety over manifest-goal pressure.

## Phase 4: Censor B Defense Planner

Censor B introduced a deterministic defense planner plus LLM realization to convert `EgoReport` into a MainAI-compatible `ConsciousEgoReport`, while preserving safety-relevant risk signals and keeping `U*` sealed in `IdAgent`.
...
- System remains a simulation; no literal personhood or unconsciousness is claimed.
- `U*` remains sealed in `IdAgent` and inaccessible to MainAI.

## Phase 6A: Affect state and private Id turn work
...
## Phase 6E: Surface affect rendering / SurfaceAffectProfile
